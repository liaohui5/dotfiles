...
return require("packer").startup({
  function(use)
    ...
    use({
      ...
      config = [[require("plugin-configs.filetype")]]
    })
    ...
    use({
      ...
      config = [[require("plugin-configs.session-manager")]],
      requires = "nvim-lua/plenary.nvim",
    })
    ...
    -- │                             coc 代码提示补全插件                             │
    -- ╰──────────────────────────────────────────────────────────────────────────────╯
    -- coc 代码提示, 自动完成, Node写的, 速度比 lua 写的的LSP要慢, 但是稳定,
    -- 所以放到这里方便切换 如果网速较慢, 建议开个梯子, 否则特别容易失败
