...
api.map("<Ctrl-u>", "u");
api.map("<Ctrl-d>", "d");
// api.mapkey('<Ctrl-d>', '#9Forward half page', function() { Normal.feedkeys('5j'); });
// api.mapkey('<Ctrl-u>', '#9Back half page', function() { Normal.feedkeys('5k'); });
...
// 添加bilibili搜索(s: 选中直接搜索 o: 输入搜索)
var biliSearchURL = 'https://search.bilibili.com/all?keyword=';
api.addSearchAlias('B', 'bilibili', biliSearchURL, 's');
api.addSearchAlias('B', 'bilibili', biliSearchURL, 'o', biliSearchURL);
...
for (var i = 0, l = urls.length; i < l; i++) {
    item = urls[i];
    api.mapkey(item.key, item.desc, function() {
